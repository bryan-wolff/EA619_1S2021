posMax = []; %vetor para posição máxima
posMin = []; %vetor para posição mínima
...
for omega=listaOmega
    ...
    posMax(end+1) = max(pos); % adiciona posição máxima no vetor posMax
    posMin(end+1) = min(pos); % adiciona posição mínima no vetor posMin
...
%plot de Mp em função da frequência
plot(listaOmega, Mp,'b',listaOmega, Mp,'r o');
hold on;
title("Máximo Sobressinal em função da frequência");
xlabel("Omega");ylabel("Mp");
set(gca,'xscale','log');
grid();
    
%plot de Mp dB em função da frequência
Mp_dB = mag2db(Mp);
plot(listaOmega, Mp_dB, 'b', listaOmega, Mp_dB, 'r o');
hold on;
title("Diagrama de Bode para o Máximo Sobressinal");
...
hold on;
bodeplot(G);legend('teóricos')
grid;
xlabel('\omega');ylabel('G(j\omega)');
title('Diagrama de Bode: Valores experimentais e teóricos')
