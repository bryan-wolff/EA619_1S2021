...
while timeSim < 2.0
    ...
    position(end+1) = posTopPlate(3) - offset;
    
    %imprime na tela
    fprintf('tempo = %.3f posicao(Z) = %.5f\n',timeSim(1,end)+timeStep,position(end));
...
plot(timeSim,position);
title('Resposta temporal do sistema para uma massa de 300g')
grid
xlabel('Tempo (s)')
ylabel('Posição na coordenada Z (m)')
xlim([0 2])
